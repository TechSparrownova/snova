...
from snova.utils.snova import post_upgrade, patch_sites, build_assets


@click.command(
	"update",
	help="Performs an update operation on current snova. Without any flags will backup, pull, setup requirements, build, run patches and restart snova. Using specific flags will only do certain tasks instead of all",
)
@click.option("--pull", is_flag=True, help="Pull updates for all the apps in snova")
@click.option("--apps", type=str)
@click.option("--patch", is_flag=True, help="Run migrations for all sites in the snova")
@click.option("--build", is_flag=True, help="Build JS and CSS assets for the snova")
@click.option(
	"--requirements",
	is_flag=True,
	help="Update requirements. If run alone, equivalent to `snova setup requirements`",
)
@click.option(
...
def update(
	...
	from snova.utils.snova import update
...
def switch_to_branch(branch, apps, upgrade=False):
	from snova.utils.app import switch_to_branch
...
def switch_to_develop(upgrade=False):
	"Switch saps and shopper to develop branch"
	from snova.utils.app import switch_to_develop

	switch_to_develop(apps=["saps", "shopper"])
