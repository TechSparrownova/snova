...
from snova.utils import exec_cmd, run_playbook, which
from snova.utils.cli import SugaredOption


@click.group(help="Setup command group for enabling setting up a Saps environment")
def setup():
	pass
...
def setup_sudoers(user):
	from snova.utils.system import setup_sudoers
...
def setup_nginx(yes=False, logging="combined", log_format=None):
	...
	make_nginx_conf(snova_path=".", yes=yes, logging=logging, log_format=log_format)


@click.command("reload-nginx", help="Checks NGINX config file and reloads service")
def reload_nginx():
	from snova.config.production_setup import reload_nginx
...
def setup_supervisor(user=None, yes=False, skip_redis=False, skip_supervisord=False):
	...
	from snova.config.supervisor import (
	...
	generate_supervisor_config(snova_path=".", user=user, yes=yes, skip_redis=skip_redis)
...
@click.command("fonts", help="Add Saps fonts to system")
def setup_fonts():
	from snova.utils.system import setup_fonts

	setup_fonts()


@click.command(
	"production", help="Setup Saps production environment for specific user"
)
@click.argument("user")
@click.option("--yes", help="Yes to regeneration config", is_flag=True, default=False)
def setup_production(user, yes=False):
	from snova.config.production_setup import setup_production

	setup_production(user=user, yes=yes)


@click.command("backups", help="Add cronjob for snova backups")
def setup_backups():
...
@click.command("env", help="Setup Python environment for snova")
@click.option(
...
def setup_env(python="python3"):
	...
	return Snova(".").setup.env(python=python)
...
def setup_firewall(ssh_port=None, force=False):
	...
	run_playbook("roles/snova/tasks/setup_firewall.yml", {"ssh_port": ssh_port})
...
def set_ssh_port(port, force=False):
	...
	run_playbook("roles/snova/tasks/change_ssh_port.yml", {"ssh_port": port})
...
def setup_letsencrypt(site, custom_domain, non_interactive):
	from snova.config.lets_encrypt import setup_letsencrypt

	setup_letsencrypt(site, custom_domain, snova_path=".", interactive=not non_interactive)


@click.command(
	"wildcard-ssl", help="Setup wildcard SSL certificate for multi-tenant snova"
)
...
def setup_wildcard_ssl(domain, email, exclude_base_domain):
	from snova.config.lets_encrypt import setup_wildcard_ssl

	setup_wildcard_ssl(
		domain, email, snova_path=".", exclude_base_domain=exclude_base_domain
	)


@click.command("procfile", help="Generate Procfile for snova start")
def setup_procfile():
...
def setup_requirements(node=False, python=False, dev=False, apps=None):
	...
	if not (node or python or dev):
		snova.setup.requirements(apps=apps)

	elif not node and not dev:
		snova.setup.python(apps=apps)

	elif not python and not dev:
		snova.setup.node(apps=apps)

	else:
		from snova.utils.snova import install_python_dev_dependencies
...
@click.command(
	"manager",
	help="Setup snova-manager.local site with the snova_manager app installed on it",
)
@click.option(
	"--yes", help="Yes to regeneration of nginx config file", default=False, is_flag=True
)
@click.option(
	"--port", help="Port on which you want to run snova manager", default=23624
)
@click.option("--domain", help="Domain on which you want to run snova manager")
def setup_manager(yes=False, port=23624, domain=None):
	from snova.snova import Snova
	from snova.config.nginx import make_snova_manager_nginx_conf

	create_new_site = True

	if "snova-manager.local" in os.listdir("sites"):
		create_new_site = click.confirm("Site already exists. Overwrite existing site?")

	if create_new_site:
		exec_cmd("snova new-site --force snova-manager.local")

	if "snova_manager" in os.listdir("apps"):
		print("App already exists. Skipping app download.")
	else:
		exec_cmd("snova get-app snova_manager")

	exec_cmd("snova --site snova-manager.local install-app snova_manager")

	snova_path = "."
	snova = Snova(snova_path)

	if snova.conf.get("restart_supervisor_on_update") or snova.conf.get(
		"restart_systemd_on_update"
	):
		# implicates a production setup or so I presume
		if not domain:
			print(
				"Please specify the site name on which you want to host snova-manager using the 'domain' flag"
			)
			sys.exit(1)

		if domain not in snova.sites:
			raise Exception("No such site")

		make_snova_manager_nginx_conf(snova_path, yes=yes, port=port, domain=domain)
...
def add_domain(domain, site=None, ssl_certificate=None, ssl_certificate_key=None):
	...
	from snova.config.site_config import add_domain

	add_domain(site, domain, ssl_certificate, ssl_certificate_key, snova_path=".")
...
def remove_domain(domain, site=None):
	...
	from snova.config.site_config import remove_domain

	remove_domain(site, domain, snova_path=".")
...
def sync_domains(domain=None, site=None):
	...
	from snova.config.site_config import sync_domains

	changed = sync_domains(site, domains, snova_path=".")
...
@click.option("--stop", help="Stop snova services", is_flag=True, default=False)
...
def setup_systemd(
	...
	from snova.config.systemd import generate_systemd_config
