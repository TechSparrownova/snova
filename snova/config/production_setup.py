...
from snova.utils import exec_cmd, which, get_snova_name, get_cmd_output, log
from snova.utils.system import fix_prod_setup_perms
...
def setup_production_prerequisites():
	...
	if not which("fail2ban-client"):
		exec_cmd("snova setup role fail2ban")
	if not which("nginx"):
		exec_cmd("snova setup role nginx")
	if not which("supervisord"):
		exec_cmd("snova setup role supervisor")


def setup_production(user, snova_path=".", yes=False):
	...
	if conf.get("restart_systemd_on_update"):
		...
		generate_systemd_config(snova_path=snova_path, user=user, yes=yes)
	else:
		print("Setting Up supervisor...")
		check_supervisord_config(user=user)
		generate_supervisor_config(snova_path=snova_path, user=user, yes=yes)

	print("Setting Up NGINX...")
	make_nginx_conf(snova_path=snova_path, yes=yes)
	fix_prod_setup_perms(snova_path, saps_user=user)
	...
	if conf.get("restart_supervisor_on_update"):
		...
		supervisor_conf = os.path.join(
			get_supervisor_confdir(), f"{snova_name}.{supervisor_conf_extn}"
		)

		# Check if symlink exists, If not then create it.
		if not os.path.islink(supervisor_conf):
			os.symlink(
				os.path.abspath(os.path.join(snova_path, "config", "supervisor.conf")),
				supervisor_conf,
			)

	if not os.path.islink(nginx_conf):
		os.symlink(
			os.path.abspath(os.path.join(snova_path, "config", "nginx.conf")), nginx_conf
		)
...
def disable_production(snova_path="."):
	snova_name = get_snova_name(snova_path)
	conf = Snova(snova_path).conf
...
def service(service_name, service_option):
	if os.path.basename(which("systemctl") or "") == "systemctl" and is_running_systemd():
		...
		service_manager = os.environ.get("SNOVA_SERVICE_MANAGER")
		if service_manager:
			service_manager_command = (
				os.environ.get("SNOVA_SERVICE_MANAGER_COMMAND")
