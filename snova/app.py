...
import snova
from snova.exceptions import NotInSnovaDirectoryError
...
class AppMeta:
	...
	def setup_details(self):
		...
		if self.snova and os.path.exists(os.path.join(self.snova.name, "apps", self.name)):
			self.mount_path = os.path.join(self.snova.name, "apps", self.name)
			self.from_apps = True
...
class App(AppMeta):
	...
	def get(self):
		...
		shallow = "--depth 1" if self.snova.shallow_clone else ""
		...
		self.snova.run(
			f"{cmd} {args}",
			cwd=os.path.join(self.snova.name, "apps"),
		)
	...
	def install(
		...
		import snova.cli
		from snova.utils.app import get_app_name
		...
		install_app(
			app=app_name,
			tag=self.tag,
			snova_path=self.snova.name,
			verbose=verbose,
			skip_assets=skip_assets,
			restart_snova=restart_snova,
	...
	def uninstall(self):
		self.snova.run(f"{self.snova.python} -m pip uninstall -y {self.name}")

	def _get_dependencies(self):
		from snova.utils.app import get_required_deps, required_apps_from_hooks
...
def make_resolution_plan(app: App, snova: "Snova"):
	"""
	...
	for app_name in app._get_dependencies():
		...
		is_valid_saps_branch(dep_app.url, dep_app.branch)
		...
		resolution.update(make_resolution_plan(dep_app, snova))
		app.local_resolution = [repo_name for repo_name, _ in reversed(resolution.items())]
	return resolution


def get_excluded_apps(snova_path="."):
	try:
		with open(os.path.join(snova_path, "sites", "excluded_apps.txt")) as f:
...
def add_to_excluded_apps_txt(app, snova_path="."):
	...
	if app not in apps:
		apps.append(app)
		return write_excluded_apps_txt(apps, snova_path=snova_path)


def write_excluded_apps_txt(apps, snova_path="."):
	with open(os.path.join(snova_path, "sites", "excluded_apps.txt"), "w") as f:
		return f.write("\n".join(apps))


def remove_from_excluded_apps_txt(app, snova_path="."):
	apps = get_excluded_apps(snova_path=snova_path)
	if app in apps:
		apps.remove(app)
		return write_excluded_apps_txt(apps, snova_path=snova_path)
...
def get_app(
	...
	"""snova get-app clones a Saps App from remote (GitHub or any other git server),
	...
	If the snova_path is not a snova directory, a new snova is created named using the
	git_url parameter.
	"""
	import snova as _snova
	import snova.cli as snova_cli
	from snova.snova import Snova
	from snova.utils.app import check_existing_dir

	snova = Snova(snova_path)
	app = App(git_url, branch=branch, snova=snova, soft_link=soft_link)
	git_url = app.url
	repo_name = app.repo
	branch = app.tag
	snova_setup = False
	restart_snova = not init_snova
	saps_path, saps_branch = None, None
	...
	if not is_snova_directory(snova_path):
		if not init_snova:
			raise NotInSnovaDirectoryError(
				...
				"Run with --init-snova if you'd like to create a Snova too."
			)
		...
		init(
			path=snova_path,
			saps_path=saps_path,
			saps_branch=saps_branch or branch,
		)
		os.chdir(snova_path)
		snova_setup = True

	if snova_setup and snova_cli.from_command_line and snova_cli.dynamic_feed:
		_snova.LOG_BUFFER.append(
			{
	...
	dir_already_exists, cloned_path = check_existing_dir(snova_path, repo_name)
	...
	if (
		...
		app.install(verbose=verbose, skip_assets=skip_assets, restart_snova=restart_snova)
...
def install_resolved_deps(
	...
	for repo_name, app in reversed(resolution.items()):
		...
		if existing_dir:
			...
			try:
				installed_branch = snova.apps.states[repo_name]["resolution"]["branch"].strip()
			except Exception:
				installed_branch = (
...
def new_app(app, no_git=None, snova_path="."):
	if snova.SAPS_VERSION in (0, None):
		raise NotInSnovaDirectoryError(
			f"{os.path.realpath(snova_path)} is not a valid snova directory."
		)
	...
	if no_git:
		if snova.SAPS_VERSION < 14:
			click.secho("Saps v14 or greater is needed for '--no-git' flag", fg="red")
			return
		args.append(no_git)

	logger.log(f"creating new app {app}")
	run_saps_cmd(*args, snova_path=snova_path)
...
def install_app(
	...
	verbose = snova_cli.verbose or verbose
	quiet_flag = "" if verbose else "--quiet"
	cache_flag = "--no-cache-dir" if no_cache else ""

	app_path = os.path.realpath(os.path.join(snova_path, "apps", app))

	snova.run(
		f"{snova.python} -m pip install {quiet_flag} --upgrade -e {app_path} {cache_flag}"
	)

	if conf.get("developer_mode"):
		install_python_dev_dependencies(apps=app, snova_path=snova_path, verbose=verbose)

	if os.path.exists(os.path.join(app_path, "package.json")):
		snova.run("yarn install", cwd=app_path)

	snova.apps.sync(app_name=app, required=resolution, branch=tag, app_dir=app_path)

	if not skip_assets:
		build_assets(snova_path=snova_path, app=app)

	if restart_snova:
		# Avoiding exceptions here as production might not be set-up
		# OR we might just be generating docker images.
		snova.reload(_raise=False)


def pull_apps(apps=None, snova_path=".", reset=False):
	"""Check all apps if there no local changes, pull"""
	from snova.snova import Snova
	from snova.utils.app import get_current_branch, get_remote

	snova = Snova(snova_path)
	rebase = "--rebase" if snova.conf.get("rebase_on_pull") else ""
	apps = apps or snova.apps
	excluded_apps = snova.excluded_apps
...
1. Temporarily remove your changes with "git stash" or discard them completely
	with "snova update --reset" or for individual repositries "git reset --hard"
	...
	for app in apps:
		...
		if os.path.exists(os.path.join(app_dir, ".git")):
			...
			if not remote:
				...
				add_to_excluded_apps_txt(app, snova_path=snova_path)
				print(
			...
			if not snova.conf.get("shallow_clone") or not reset:
				...
				if is_shallow:
					...
					snova.run(f"git fetch {remote} --unshallow", cwd=app_dir)

			branch = get_current_branch(app, snova_path=snova_path)
			logger.log(f"pulling {app}")
			if reset:
				reset_cmd = f"git reset --hard {remote}/{branch}"
				if snova.conf.get("shallow_clone"):
					snova.run(f"git fetch --depth=1 --no-tags {remote} {branch}", cwd=app_dir)
					snova.run(reset_cmd, cwd=app_dir)
					snova.run("git reflog expire --all", cwd=app_dir)
					snova.run("git gc --prune=all", cwd=app_dir)
				else:
					snova.run("git fetch --all", cwd=app_dir)
					snova.run(reset_cmd, cwd=app_dir)
			else:
				snova.run(f"git pull {rebase} {remote} {branch}", cwd=app_dir)
			snova.run('find . -name "*.pyc" -delete', cwd=app_dir)
...
def get_repo_dir(app, snova_path="."):
	return os.path.join(snova_path, "apps", app)
...
def install_apps_from_path(path, snova_path="."):
